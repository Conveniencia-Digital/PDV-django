...
document.body.addEventListener("htmx:afterSwap", function (evt) {
  ...
  function verificarDuplicatas() {
    ...
    produtos.forEach(produto => {
        ...
        if (valor !== "Selecione" && valor !== "") {
            if (produtosSelecionados.has(valor)) {
                alert("Este produto já foi adicionado! O item será removido.");
                
                // Remove a linha do formulário automaticamente
                if (itemRow) {
                    itemRow.remove();
                    reorderItems(); // Atualiza os índices
                }
    ...
    return !duplicatas; // Retorna falso se houver duplicatas
}

// ✅ Bloqueia o envio do formulário se houver duplicatas
...
// ✅ Chama a função quando um novo produto é adicionado
...
// ✅ Aciona a função sempre que o usuário altera um produto
...
// ✅ Reorganiza os índices do formset
function reorderItems() {
    ...
    items.forEach((item, i) => {
        ...
        let vendasField = item.querySelector('[data-field="vendas"]');
        let produtoField = item.querySelector('[data-field="produto"]');
        let quantidadeField = item.querySelector('[data-field="quantidade"]');
        let precoField = item.querySelector('[data-field="preco"]');

        if (vendasField) {
            vendasField.setAttribute('name', `items-${i}-vendas`);
            vendasField.setAttribute('id', `id_items-${i}-vendas`);
        }

        if (produtoField) {
            produtoField.setAttribute('name', `items-${i}-produto`);
            produtoField.setAttribute('id', `id_items-${i}-produto`);
            produtoField.setAttribute('hx-target', `#id_items-${i}-preco`);
        }

        if (quantidadeField) {
            quantidadeField.setAttribute('name', `items-${i}-quantidade`);
        }

        if (precoField) {
            precoField.setAttribute('name', `items-${i}-preco`);
            precoField.setAttribute('id', `id_items-${i}-preco`);

            let preco = parseFloat(precoField.value) || 0;
            let qtd = parseInt(quantidadeField.value) || 1;
    ...
    // Processa o htmx para reconhecer os novos elementos
